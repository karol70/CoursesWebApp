...
import { urlCities, urlTypes } from "../endpoints";
...
export default function CourseForm(props: courseFormProps) {
  ...
  useEffect(() => {
    axios.get(urlTypes).then((response: AxiosResponse<typesDTO[]>) => {
      setTypes(response.data);
    });
  }, []);

  useEffect(() => {
    axios.get(urlCities).then((response: AxiosResponse<citiesDTO[]>) => {
    ...
    >
      {(formikProps) => (
        <Form>
          ...
          />
          <TextField
            displayName="Link do strony głownej kursu"
            field="mainPage"
          />
...
interface courseFormProps {
  ...
  onSubmit(
    values: courseCreationDTO,
    actions: FormikHelpers<courseCreationDTO>
  ): void;
  title: string;
