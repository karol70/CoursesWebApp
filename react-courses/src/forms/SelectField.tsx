...
export default function SelectField(props: selectFieldProps){
    return(
        <div className='mb-3'>
            ...
            <Field id={props.field} as="select" className="form-select" {...props.other.getFieldProps(props.field)}>
                <option>{props.message}</option>                                                                                                                 
                {props.options.map(prop=> <option key={prop.id} value={prop.id}>{prop.name}
                </option>)}                       
            </Field>
